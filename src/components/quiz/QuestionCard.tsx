...
import { ensureQuestionPerformance, updateQuestionPerformance } from '@/lib/srs-storage';
import { calculateNextReview } from '@/lib/srs-algorithm';


interface QuestionCardProps {
  question: Question;
  onAnswer: (selectedAnswerId: string, timeTaken: number, confidence?: ConfidenceLevel) => void;
...
export function QuestionCard({ question, onAnswer, onTimeout, onNext, questionNumber, totalQuestions }: QuestionCardProps) {
  ...
  useEffect(() => {
    if (isAnsweredRef.current) {
      ...
      // Only auto-advance for skipped/timed out questions
      // Correct answers wait for confidence submission
      // Incorrect answers auto-advance immediately (they get auto-rated as GUESS)
      if (wasSkippedOrTimedOut) {
  ...
  const handleConfidenceSubmit = async (confidence: ConfidenceLevel) => {
    ...
    if (currentUser && question.categoryId) {
      try {
        const perf = await ensureQuestionPerformance(
          question.id,
          currentUser.uid,
          question.categoryId
        );
    ...
    <>
      <Card className="w-full max-w-3xl mx-auto shadow-xl transition-all duration-300 ease-in-out">
        ...
        <CardFooter className="flex flex-col items-center pt-3 space-y-3 px-4 sm:px-6 pb-4 sm:pb-6">
          ...
          {isAnswered && autoAdvanceMessage && !showConfidenceSlider && (
          ...
          <div className="flex flex-col sm:flex-row gap-2 w-full justify-center">
            ...
            {isAnswered && question.explanation && !showConfidenceSlider && (
              <Button
                onClick={() => {
            ...
            {isAnswered && !showConfidenceSlider && (
              <Button
