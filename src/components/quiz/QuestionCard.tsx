...
export function QuestionCard({ question, onAnswer, onTimeout, onNext, questionNumber, totalQuestions }: QuestionCardProps) {
  ...
  const handleShowExplanation = async () => {
    ...
    if (!correctAnswerOption) {
        toast({ title: "Error", description: "Could not find correct answer details.", variant: "destructive"});
        setIsExplanationLoading(false);
        return;
    }
  ...
  return (
    <>
      <Card className="w-full max-w-3xl mx-auto shadow-xl transition-all duration-300 ease-in-out">
          ...
          {question.options.map((option) => (
            <Button
              ...
              className={cn(
                ...
                isAnswered && !selectedAnswerId && option.id === question.correctAnswerId && "ring-2 ring-offset-2 ring-destructive" 
              )}
        ...
        <CardFooter className="flex flex-col items-center pt-4 space-y-3">
